...
export default function Dashboard({ apiBase, token, onLogout, user }) {
  ...
  const handleBooking = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Reload bookings after booking is created
        setTimeout(() => {
          loadBookings();
          loadCars(); // Also refresh cars list to update availability
        }, 500);
      } else {
  ...
  const loadBookings = async () => {
    try {
      ...
      if (!user || !user.id) {
        console.warn('No user ID available for loading bookings', user);
        setBookings([]);
        return;
      }
      ...
      const url = `${apiBase}/bookings/user/${user.id}?t=${timestamp}`;
      console.log('URL:', url);
                ...
                {bookings.map((booking) => {
                  ...
                  const getStatusMessage = (st) => {
                    switch(st) {
                      ...
                      case 'confirmed': return 'Admin approved! Ready to pick up';
          ...
          <div className="modal-content transaction-receipt" onClick={(e) => e.stopPropagation()}>
            ...
            <button 
              ...
              onClick={() => {
                ...
                // Automatically open Your Bookings popover after done
                setTimeout(() => {
