...
export default function AdminDashboard({ apiBase, token, onLogout }) {
  ...
  const loadBookings = async () => {
    ...
    try {
      ...
      const res = await fetch(`${apiBase}/bookings?t=${timestamp}`, { headers: authHeaders });
      const data = await res.json();
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          // Reload both bookings and cars to reflect changes
          // If cancelled, car becomes available; if confirmed, car becomes unavailable
          await loadBookings();
          await loadCars();
                ...
                <tbody>
                  {cars.map((car) => (
                    <tr key={car.id}>
                      ...
                      <td>
                        <span className={car.availability ? 'available' : 'unavailable'} style={{ fontWeight: '600', fontSize: '14px' }}>
