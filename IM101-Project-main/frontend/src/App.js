...
function App() {
  ...
  const handleSearch = async (searchTerm = '', redirect = false) => {
    try {
      ...
      // dedupe by model+make (case-insensitive) to avoid duplicate cards
      const seen = new Set();
      const uniqueFiltered = filteredItems.filter((c) => {
        const key = `${(c.model||'').toString().toLowerCase().trim()}|${(c.make||'').toString().toLowerCase().trim()}`;
  ...
  // Populate initial car list on mount so HomePage renders backend data
  useEffect(() => {
    handleSearch();
  ...
  const handleAdminLogin = async (e) => {
    ...
    try {
      ...
      if (data.token && data.user) {
        ...
        console.log('✅ Admin verified, setting state...');
        setAdminToken(data.token);
        setIsAdminMode(true);
        localStorage.setItem('adminToken', data.token);
        console.log('✅ State updated - isAdminMode: true, token saved');
        setAdminEmail('');
        setAdminPassword('');
